 ...
 * @property Prise[] $prise
 * @property integer not_prom
 * @property  string alt
...
class Gods extends \yii\db\ActiveRecord
{
    ...
    const NOT_HAVE=1;
    const HAVE=0;
    const EXEPT='барабашиво';

    const SCENARIO_SEASSON = 'seasson';

    const NOT_PROM=0;
    const PROM_YES=1;

    public static $promStatus=[
      self::NOT_PROM=>'Неставить в пром',
      self::PROM_YES=>'Ставить в пром'
    ];

    public static $haveName=[
        self::NOT_HAVE=>'Отсутсвует',
        self::HAVE=>'В наличие'
    ];
    /**
     * @inheritdoc
    ...
    public function getHaveNames()
    {
        return self::$haveName[$this->have];
    }

    public function getStatusProm(){
        return self::$promStatus[$this->not_prom];
    }
    ...
    public function scenarios()
    {
        ...
        $scenarios[self::SCENARIO_SEASSON] = ['id','seasons'];
        return $scenarios;
    ...
    {
        return (Yii::$app->controller->id == 'goods' && (Yii::$app->controller->action->id == 'create' || Yii::$app->controller->action->id == 'update'));
    }
    public function afterSave($insert, $changedAttributes)
     {
         ...
         if(!empty($this->seasons)){
             Addlfeild::deleteAll([
                 'AND',
                 ['id_gods'=>$this->id],
                 ['IN','key_feild',['winter','spring','summer','fall']],
                 ['NOT IN','key_feild',$this->seasons],
             ]);
             foreach ($this->seasons as $season){
                 if(empty(Addlfeild::find()->where(['id_gods'=>$this->id,'key_feild'=>$season])->one())){
                     ...
                     if(!$addField->save()){
                         throw new \RuntimeException(var_dump($addField->errors));
                     };
                 }
    ...
    public function getKeywords(){
        return $this->hasOne(Addlfeild::className(),['id_gods'=>'id'])->andWhere(['key_feild'=>'keywords']);
    }
                    ...
                    if(isset($model->category)&&$model->have==0) {
                        if(isset($model->price_selling)) {
                            return new promOffer([
                                ...
                                'available' => $model->getAvailabel($model->have),
                                'url' => $model->getUrl($model->slug_gods),
                                'price' => (!empty($model->prise))?$model->getNewPrice($model->price_selling, $model->prise->price1):0,
                                'delivery' => $model->getDivelery($model->addfeilds),
                                'vendorCode' => $model->getVendorCode($model->addfeilds),
                                'currencyId' => 'UAH',
                                'pickup' => true,
                                //'vendor'=>'Милый дом',
                                'oldprice' => (!empty($model->prise))?$this->getThePrice($model->prise):0,
                                'picture' => $model->getPicture($model->images),
                                'country_of_origin' => 'Украина',
                                'param' => $model->getParams($model->addfeilds),
                            ...
                            return new promOffer([
                                ...
                                'url' => $model->getUrl($model->slug_gods),
                                //'vendor'=>'Милый дом',
                                'price' => (!empty($model->prise))?$this->getThePrice($model->prise):0,
                                'delivery' => $model->getDivelery($model->addfeilds),
                                'vendorCode' => $model->getVendorCode($model->addfeilds),
                                'currencyId' => 'UAH',
                                'pickup' => true,
    ...
    public function getSessons(){
        $sessons=Addlfeild::find()->where(['id_gods'=>$this->id])->andWhere(['in','key_feild',array_keys(Addlfeild::$season)])->all();
        Yii::error($sessons);
        if(!empty($sessons)){
            foreach ($sessons as $sesson){
              $arrSezons[]=$sesson->sessonField;
            }
        }

        if(!empty($arrSezons))
            return implode('<br/>',$arrSezons);
        return null;
    }
    ...
    {
        return [
            [['title', 'discription_gods', 'create_at','have',], 'required'],
            [['discription_gods'], 'string'],
            [['color','pregmath','currency','seasons'],'safe'],
            ['id_prise','required','message' => 'Не указана цена'],
            [['id_prise', 'create_at', 'upedate_at','price_selling','have','pluss','manufacturer_price','not_prom','price'], 'integer'],
            [['title','quote','alt'], 'string', 'max' => 255],
